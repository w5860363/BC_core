...
void boss_julianneAI::DamageTaken(Unit* /*done_by*/, uint32 &damage)
{
    ...
    {
        error_log("BSCR: boss_julianneAI: cannot take damage in PHASE_ROMULO, why was i here?");
        damage = 0;
        return;
    }
    ...
    error_log("BSCR: boss_julianneAI: DamageTaken reach end of code, that should not happen.");
}
...
void boss_romuloAI::DamageTaken(Unit* /*done_by*/, uint32 &damage)
{
    ...
    error_log("BSCR: boss_romuloAI: DamageTaken reach end of code, that should not happen.");
}
