...
struct npc_barnesAI : public npc_escortAI
{
    ...
    {
        debug_log("BSCR: Barnes Opera Event - Introduction complete - preparing encounter %d", m_uiEventId);
        uint8 index = 0;
        uint8 count = 0;
...
bool GossipSelect_npc_barnes(Player* pPlayer, Creature* pCreature, uint32 /*uiSender*/, uint32 uiAction)
{
    ...
    switch (uiAction)
    {
        ...
        case GOSSIP_ACTION_INFO_DEF+3:
            ...
            outstring_log("BSCR: player (GUID %llu) manually set Opera event to EVENT_OZ",pPlayer->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF+4:
            pPlayer->CLOSE_GOSSIP_MENU();
            pBarnesAI->m_uiEventId = EVENT_HOOD;
            outstring_log("BSCR: player (GUID %llu) manually set Opera event to EVENT_HOOD",pPlayer->GetGUID());
            break;
        case GOSSIP_ACTION_INFO_DEF+5:
            pPlayer->CLOSE_GOSSIP_MENU();
            pBarnesAI->m_uiEventId = EVENT_RAJ;
            outstring_log("BSCR: player (GUID %llu) manually set Opera event to EVENT_RAJ",pPlayer->GetGUID());
            break;
    }
