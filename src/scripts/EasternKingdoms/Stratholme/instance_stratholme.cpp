...
struct instance_stratholme : public ScriptedInstance
{
    ...
    bool StartSlaugtherSquare()
    {
        ...
        debug_log("BSCR: Instance Stratholme: Cannot open slaugther square yet.");
        return false;
    }
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        case TYPE_BARON_RUN:
            ...
            case IN_PROGRESS:
                ...
                debug_log("BSCR: Instance Stratholme: Baron run in progress.");
                break;
            case FAIL:
        ...
        case TYPE_RAMSTEIN:
            if (data == IN_PROGRESS)
            {
                ...
                if (!count)
                {
                    ...
                    debug_log("BSCR: Instance Stratholme: Ramstein spawned.");
                }
                else
                    debug_log("BSCR: Instance Stratholme: %u Abomnation left to kill.",count);
            }
            ...
            if (data == DONE)
            {
                ...
                debug_log("BSCR: Instance Stratholme: Slaugther event will continue in 5 minutes.");
            }
            Encounter[4] = data;
            break;
        case TYPE_BARON:
    ...
    void Update(uint32 diff)
    {
        if (BaronRun_Timer)
        {
            if (BaronRun_Timer <= diff)
            {
                ...
                BaronRun_Timer = 0;
                debug_log("BSCR: Instance Stratholme: Baron run event reached end. Event has state %u.",GetData(TYPE_BARON_RUN));
        ...
        if (SlaugtherSquare_Timer)
        {
            if (SlaugtherSquare_Timer <= diff)
            {
                if (Creature* pBaron = instance->GetCreature(baronGUID))
                {
                    ...
                    HandleGameObject(ziggurat4GUID, true);
                    HandleGameObject(ziggurat5GUID, true);
                    debug_log("BSCR: Instance Stratholme: Black guard sentries spawned. Opening gates to baron.");
                }
                SlaugtherSquare_Timer = 0;
