...
struct instance_zulaman : public ScriptedInstance
{
    ...
    std::string GetSaveData()
    {
        ...
        //error_log("BSCR: Zul'aman saved, %s.", data);
        return data;
    }
    ...
    void Load(const char* load)
    {
        ...
        //error_log("BSCR: Zul'aman loaded, %s.", ss.str().c_str());
        char dataHead; // S
        uint16 data1, data2, data3;
        ss >> dataHead >> data1 >> data2 >> data3;
        //error_log("BSCR: Zul'aman loaded, %d %d %d.", data1, data2, data3);
        if (dataHead == 'S')
        {
            BossKilled = data1;
            ChestLooted = data2;
            QuestMinute = data3;
        } else error_log("BSCR: Zul'aman: corrupted save data.");
    }
