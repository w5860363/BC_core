...
struct boss_kaelthasAI : public ScriptedAI
{
    ...
    void StartEvent()
    {
        ...
        {
            error_log("BSCR: Kael'Thas One or more advisors missing, Skipping Phases 1-3");
            ...
            case 2:
            {
                ...
                if (PhaseSubphase == 1)
                {
                    ...
                    for (uint32 i = 0; i < 7; ++i)
                    {
                        ...
                        if (!Weapon)
                            error_log("SBSCR: Kael'thas weapon %i could not be spawned", i);
                        else
                        {
            ...
            case 3:
            {
                if (PhaseSubphase == 0)
                {
                    ...
                    for (uint32 i = 0; i < 4; ++i)
                    {
                        ...
                        if (!Advisor)
                            error_log("BSCR: Kael'Thas Advisor %u does not exist. Possibly despawned? Incorrectly Killed?", i);
                        else if (pTarget)
