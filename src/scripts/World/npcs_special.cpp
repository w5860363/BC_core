    ...
    if (Event)
    {
        if (SummonPatient_Timer <= diff)
        {
            ...
            switch (me->GetEntry())
            {
                ...
                default:
                    error_log("BSCR: Invalid entry for Triage doctor. Please check your database");
                    return;
            }
...
bool GossipHello_npc_spiritofazuregos(Player* player, Creature* creature) 
{ 
if (player->GetQuestStatus(8575) == QUEST_STATUS_NONE && !player->HasItemCount(20949, 1)) 
                player->PrepareQuestMenu(creature->GetGUID()); 
        player->ADD_GOSSIP_ITEM(0, "Do you want the Magical Ledger?", GOSSIP_SENDER_MAIN, GOSSIP_ACTION_INFO_DEF+1); 
...
bool GossipSelect_npc_spiritofazuregos(Player* player, Creature* creature, uint32 sender, uint32 action) 
{ 
        ...
        { 
         case GOSSIP_ACTION_INFO_DEF+1: 
                { 
                        msg = player->CanStoreNewItem(NULL_BAG, NULL_SLOT, dest, 20949, 1); 
                        if (msg == EQUIP_ERR_OK) 
                        { 
                                player->StoreNewItem(dest, 20949, true); 
                                creature->MonsterWhisper("Operation done!", player->GetGUID()); 
                        } 
                        else 
                        { 
                                creature->MonsterWhisper("You can't get more Magical Ledger!", player->GetGUID()); 
                        } 
                } 
