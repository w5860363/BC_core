...
struct instance_dark_portal : public ScriptedInstance
{
    ...
    Player* GetPlayerInMap()
    {
        ...
        debug_log("BSCR: Instance Black Portal: GetPlayerInMap, but PlayerList is empty!");
        return NULL;
    }
    ...
    void UpdateBMWorldState(uint32 id, uint32 state)
    {
        ...
        } else debug_log("BSCR: Instance Black Portal: UpdateBMWorldState, but PlayerList is empty!");
    }
    ...
    void SetData(uint32 type, uint32 data)
    {
        ...
        if (!player)
        {
            debug_log("BSCR: Instance Black Portal: SetData (Type: %u Data %u) cannot find any player.", type, data);
            return;
        }
        ...
        case TYPE_MEDIVH:
            ...
            else
            {
                if (data == IN_PROGRESS)
                {
                    debug_log("BSCR: Instance Dark Portal: Starting event.");
                    InitWorldState();
    ...
    Unit* SummonedPortalBoss(Unit* source)
    {
        ...
        debug_log("BSCR: Instance Dark Portal: Summoning rift boss entry %u.",entry);
        ...
        debug_log("BSCR: Instance Dark Portal: what just happened there? No boss, no loot, no fun...");
        return NULL;
    }
    ...
    void DoSpawnPortal()
    {
        ...
        if (Unit* medivh = Unit::GetUnit(*player,MedivhGUID))
        {
            for (uint8 i = 0; i < 4; i++)
            {
                ...
                {
                    debug_log("BSCR: Instance Dark Portal: Creating Time Rift at locationId %i (old locationId was %u).",tmp,CurrentRiftId);
