...
void FollowerAI::EnterEvadeMode()
{
    ...
    {
        sLog.outDebug("BSCR: FollowerAI left combat, returning to CombatStartPosition.");
...
void FollowerAI::UpdateAI(const uint32 uiDiff)
{
    if (HasFollowState(STATE_FOLLOW_INPROGRESS) && !me->getVictim())
    {
        if (m_uiUpdateFollowTimer <= uiDiff)
        {
            ...
            {
                sLog.outDebug("BSCR: FollowerAI is set completed, despawns.");
                me->ForcedDespawn();
                return;
            }
            ...
            if (Player* pPlayer = GetLeaderForFollower())
            {
                ...
                {
                    sLog.outDebug("BSCR: FollowerAI is returning to leader.");
            ...
            {
                sLog.outDebug("BSCR: FollowerAI failed because player/group was to far away or not found");
                me->ForcedDespawn();
                return;
            }
...
void FollowerAI::StartFollow(Player* pLeader, uint32 uiFactionForFollower, const Quest* pQuest)
{
    if (me->getVictim())
    {
        sLog.outDebug("BSCR: FollowerAI attempt to StartFollow while in combat.");
        return;
    }

    if (HasFollowState(STATE_FOLLOW_INPROGRESS))
    {
        sLog.outError("BSCR: FollowerAI attempt to StartFollow while already following.");
        return;
    }
    ...
    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        ...
        sLog.outDebug("BSCR: FollowerAI start with WAYPOINT_MOTION_TYPE, set to MoveIdle.");
    }
    ...
    sLog.outDebug("BSCR: FollowerAI start follow %s (GUID " UI64FMTD ")", pLeader->GetName(), m_uiLeaderGUID);
}
...
Player* FollowerAI::GetLeaderForFollower()
{
        ...
        else
        {
            if (Group* pGroup = pLeader->GetGroup())
            {
                for (GroupReference* pRef = pGroup->GetFirstMember(); pRef != NULL; pRef = pRef->next())
                {
                    ...
                    {
                        sLog.outDebug("BSCR: FollowerAI GetLeader changed and returned new leader.");
    ...
    sLog.outDebug("BSCR: FollowerAI GetLeader can not find suitable leader.");
    return NULL;
}
