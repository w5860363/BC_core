...
void Pet::GivePetXP(uint32 xp)
{
    ...
    while (newXP >= nextLvlXP && level < sWorld.getConfig(CONFIG_MAX_PLAYER_LEVEL))
    {
        ...
        SetLevel(level + 1);
        SetUInt32Value(UNIT_FIELD_PETNEXTLEVELEXP, uint32((BlizzLike::XP::xp_to_level(level+1))/4));
...
bool Pet::CreateBaseAtCreature(Creature* creature)
{
    ...
    SetUInt32Value(UNIT_FIELD_PETNEXTLEVELEXP, uint32((BlizzLike::XP::xp_to_level(creature->getLevel()))/4));
    SetUInt32Value(UNIT_FIELD_FLAGS, UNIT_FLAG_PVP_ATTACKABLE);
...
bool Guardian::InitStatsForLevel(uint32 petlevel)
{
    ...
    switch (petType)
    {
        ...
        case HUNTER_PET:
        {
            SetUInt32Value(UNIT_FIELD_PETNEXTLEVELEXP, uint32((BlizzLike::XP::xp_to_level(petlevel))/4));
