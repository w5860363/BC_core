    ...
    for (std::vector<uint32>::iterator itr = SendDataId.begin(); itr != SendDataId.end(); ++itr)
    {
        ...
        switch (type)
        {
            case MEM_CHECK:
            {
                ...
                if (Mem_Result != 0)
                {
                    sLog.outDebug("WARDEN: RESULT MEM_CHECK not 0x00 at this stage, CheckId %u account Id %u", rd->id, Client->GetAccountId());
                    continue;
                }
                ...
                buff.rpos(buff.rpos() + rd->Length);
                sLog.outDebug("WARDEN: RESULT MEM_CHECK passed CheckId %u account Id %u", rd->id, Client->GetAccountId());
                break;
            }
            case PAGE_CHECK_A:
            case PAGE_CHECK_B:
            ...
            case MODULE_CHECK:
            {
                ...
                if (memcmp(buff.contents() + buff.rpos(), &byte, sizeof(uint8)) != 0)
                {
                    if (type == PAGE_CHECK_A || type == PAGE_CHECK_B)
                        sLog.outWarden("PAGE_CHECK fail, CheckId %u account Id %u", rd->id, Client->GetAccountId());
                    if (type == MODULE_CHECK)
                        sLog.outWarden("MODULE_CHECK fail, CheckId %u account Id %u", rd->id, Client->GetAccountId());
                    if (type == DRIVER_CHECK)
                        sLog.outWarden("DRIVER_CHECK fail, CheckId %u account Id %u", rd->id, Client->GetAccountId());
                    found = true;
            ...
            case LUA_STR_CHECK:
            {
                ...
                if (Lua_Result != 0)
                {
                    sLog.outWarden("LUA_STR_CHECK fail, CheckId %u account Id %u", rd->id, Client->GetAccountId());
                    found = true;
                    continue;
                }
            ...
            case MPQ_CHECK:
            {
                ...
                if (Mpq_Result != 0)
                {
                    sLog.outWarden("MPQ_CHECK not 0x00 account id %u", Client->GetAccountId());
                    found = true;
                    continue;
                }
                ...
                {
                    sLog.outWarden("MPQ_CHECK fail, CheckId %u account Id %u", rd->id, Client->GetAccountId());
                    found = true;
