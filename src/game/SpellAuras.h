...
#ifndef BLIZZLIKE_SPELLAURAS_H
#define BLIZZLIKE_SPELLAURAS_H
...
//
// Other case choice: each code line moved under if (Real) check is blizzlike speedup,
//      each setting object update field code line moved under if (Real) check is significant blizzlike speedup, and less server->client data sends
//      each packet sending code moved under if (Real) check is _large_ blizzlike speedup, and lot less server->client data sends
