...
void npc_escortAI::EnterEvadeMode()
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING))
    {
        ...
        ReturnToLastPoint();
        debug_log("BSCR: EscortAI has left combat and is now returning to last point");
    }
    else
    {
...
void npc_escortAI::UpdateAI(const uint32 uiDiff)
{
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && !me->getVictim() && m_uiWPWaitTimer && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiWPWaitTimer <= uiDiff)
        {
            ...
            if (CurrentWP == WaypointList.end())
            {
                if (DespawnAtEnd)
                {
                    debug_log("BSCR: EscortAI reached end of waypoints");
                    ...
                    if (m_bCanReturnToStart)
                    {
                        ...
                        debug_log("BSCR: EscortAI are returning home to spawn location: %u, %f, %f, %f", POINT_HOME, fRetX, fRetY, fRetZ);
                        return;
                    }
                ...
                {
                    debug_log("BSCR: EscortAI reached end of waypoints with Despawn off");
            ...
            if (!HasEscortState(STATE_ESCORT_PAUSED))
            {
                ...
                debug_log("BSCR: EscortAI start waypoint %u (%f, %f, %f).", CurrentWP->id, CurrentWP->x, CurrentWP->y, CurrentWP->z);
    ...
    if (HasEscortState(STATE_ESCORT_ESCORTING) && m_uiPlayerGUID && !me->getVictim() && !HasEscortState(STATE_ESCORT_RETURNING))
    {
        if (m_uiPlayerCheckTimer <= uiDiff)
        {
            ...
            {
                debug_log("BSCR: EscortAI failed because player/group was to far away or not found");
...
void npc_escortAI::MovementInform(uint32 uiMoveType, uint32 uiPointId)
{
    ...
    {
        debug_log("BSCR: EscortAI has returned to original position before combat");
    ...
    {
        debug_log("BSCR: EscortAI has returned to original home location and will continue from beginning of waypoint list.");
    ...
    else
    {
        ...
        {
            error_log("BSCR ERROR: EscortAI reached waypoint out of order %u, expected %u", uiPointId, CurrentWP->id);
            return;
        }

        debug_log("BSCR: EscortAI Waypoint %u reached", CurrentWP->id);
...
void npc_escortAI::AddWaypoint(uint32 id, float x, float y, float z, uint32 WaitTimeMs)
{
    ...
    // i think BSCR no longer uses this function
    ScriptWP = true;
    /*PointMovement wp;
...
void npc_escortAI::SetRun(bool bRun)
{
    if (bRun)
    {
        if (!m_bIsRunning)
            ...
            debug_log("BSCR: EscortAI attempt to set run mode, but is already running.");
    }
    else
    {
        if (m_bIsRunning)
            ...
            debug_log("BSCR: EscortAI attempt to set walk mode, but is already walking.");
    }
    m_bIsRunning = bRun;
}
...
void npc_escortAI::Start(bool bIsActiveAttacker, bool bRun, uint64 uiPlayerGUID, const Quest* pQuest, bool bInstantRespawn, bool bCanLoopPath)
{
    ...
    if (!ScriptWP) // BSCR never adds wp in script, but tc does
    {
    ...
    if (WaypointList.empty())
    {
        error_db_log("BSCR: EscortAI Start with 0 waypoints (possible missing entry in script_waypoint. Quest: %u).", pQuest ? pQuest->GetQuestId() : 0);
        return;
    }
    ...
    if (m_bCanReturnToStart && m_bCanInstantRespawn)
        debug_log("BSCR: EscortAI is set to return home after waypoint end and instant respawn at waypoint end. Creature will never despawn.");

    if (me->GetMotionMaster()->GetCurrentMovementGeneratorType() == WAYPOINT_MOTION_TYPE)
    {
        me->GetMotionMaster()->MovementExpired();
        me->GetMotionMaster()->MoveIdle();
        debug_log("BSCR: EscortAI start with WAYPOINT_MOTION_TYPE, changed to MoveIdle.");
    }
