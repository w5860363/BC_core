...
void ObjectMgr::AddCreatureToGrid(uint32 guid, CreatureData const* data)
{
    ...
    for (uint8 i = 0; mask != 0; i++, mask >>= 1)
    {
        if (mask & 1)
        {
            CellPair cell_pair = BlizzLike::ComputeCellPair(data->posX, data->posY);
...
void ObjectMgr::LoadQuests()
{
    ...
    for (QuestMap::iterator iter = mQuestTemplates.begin(); iter != mQuestTemplates.end(); ++iter)
    {
        ...
        if (qinfo->QuestFlags & ~QUEST_BLIZZLIKE_FLAGS_DB_ALLOWED)
        {
            sLog.outErrorDb("Quest %u has SpecialFlags = %u > max allowed value. Correct SpecialFlags to value <= %u",
                qinfo->GetQuestId(),qinfo->QuestFlags,QUEST_BLIZZLIKE_FLAGS_DB_ALLOWED >> 16);
            qinfo->QuestFlags &= QUEST_BLIZZLIKE_FLAGS_DB_ALLOWED;
        }

        if (qinfo->QuestFlags & QUEST_FLAGS_DAILY)
        {
            if (!(qinfo->QuestFlags & QUEST_BLIZZLIKE_FLAGS_REPEATABLE))
            {
                sLog.outErrorDb("Daily Quest %u not marked as repeatable in SpecialFlags, added.",qinfo->GetQuestId());
                qinfo->QuestFlags |= QUEST_BLIZZLIKE_FLAGS_REPEATABLE;
            }
        }
        ...
        for (int j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                qinfo->SetFlag(QUEST_BLIZZLIKE_FLAGS_DELIVER);
        ...
        for (int j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                if (!qinfo->ReqCreatureOrGOId[j])
                {
                    ...
                    if (found)
                    {
                        if (!qinfo->HasFlag(QUEST_BLIZZLIKE_FLAGS_EXPLORATION_OR_EVENT))
                        {
                            sLog.outErrorDb("Spell (id: %u) has SPELL_EFFECT_QUEST_COMPLETE or SPELL_EFFECT_SEND_EVENT for quest %u and ReqCreatureOrGOId%d = 0, but quest does not have flag QUEST_BLIZZLIKE_FLAGS_EXPLORATION_OR_EVENT. Quest flags or ReqCreatureOrGOId%d must be fixed, quest modified to enable objective.",spellInfo->Id,qinfo->QuestId,j+1,j+1);

                            // this will prevent quest completing without objective
                            //const_cast<Quest*>(qinfo)->SetFlag(QUEST_BLIZZLIKE_FLAGS_EXPLORATION_OR_EVENT);
                        }
                    }
                    else
                    {
        ...
        for (int j = 0; j < QUEST_OBJECTIVES_COUNT; ++j)
        {
            ...
            if (id)
            {
                ...
                qinfo->SetFlag(QUEST_BLIZZLIKE_FLAGS_KILL_OR_CAST | QUEST_BLIZZLIKE_FLAGS_SPEAKTO);
        ...
        if (qinfo->LimitTime)
            qinfo->SetFlag(QUEST_BLIZZLIKE_FLAGS_TIMED);
    }

    // check QUEST_BLIZZLIKE_FLAGS_EXPLORATION_OR_EVENT for spell with SPELL_EFFECT_QUEST_COMPLETE
    ...
    do
    {
        ...
        switch (tmp.command)
        {
            ...
            case SCRIPT_COMMAND_TELEPORT_TO:
            {
                ...
                if (!BlizzLike::IsValidMapCoord(tmp.TeleportTo.DestX, tmp.TeleportTo.DestY, tmp.TeleportTo.DestZ, tmp.TeleportTo.Orientation))
                {
            ...
            case SCRIPT_COMMAND_QUEST_EXPLORED:
            {
                ...
                {
                    sLog.outErrorDb("Table `%s` has quest (ID: %u) in SCRIPT_COMMAND_QUEST_EXPLORED in `datalong` for script id %u, but quest not have flag QUEST_BLIZZLIKE_FLAGS_EXPLORATION_OR_EVENT in quest flags. Script command or quest flags wrong. Quest modified to require objective.",
                        tableName.c_str(), tmp.QuestExplored.QuestID, tmp.id);
            ...
            {
                if (!BlizzLike::IsValidMapCoord(tmp.TempSummonCreature.PosX, tmp.TempSummonCreature.PosY, tmp.TempSummonCreature.PosZ, tmp.TempSummonCreature.Orientation))
                {
...
void ObjectMgr::LoadQuestAreaTriggers()
{
    ...
    do
    {
        ...
        if (!quest->HasFlag(QUEST_BLIZZLIKE_FLAGS_EXPLORATION_OR_EVENT))
        {
            sLog.outErrorDb("Table areatrigger_involvedrelation has record (id: %u) for not quest %u, but quest does not have flag QUEST_BLIZZLIKE_FLAGS_EXPLORATION_OR_EVENT. Trigger or quest flags must be fixed, quest modified to require objective.",trigger_ID,quest_ID);
...
bool ObjectMgr::LoadblizzlikeStrings(DatabaseType& db, char const* table, int32 min_value, int32 max_value)
{
    ...
    for (blizzlikeStringLocaleMap::iterator itr = mblizzlikeStringLocaleMap.begin(); itr != mblizzlikeStringLocaleMap.end();)
    {
        if (itr->first >= start_value && itr->first < end_value)
            mblizzlikeStringLocaleMap.erase(itr++);
        else
            ++itr;
    }
    ...
    if (!result)
    {
        ...
        sLog.outString();
        if (min_value == MIN_BLIZZLIKE_STRING_ID)              // error only in case internal strings
            sLog.outErrorDb(">> Loaded 0 blizzlike strings. DB table %s is empty. Cannot continue.",table);
        else
    ...
    do
    {
        ...
        blizzlikeStringLocale& data = mblizzlikeStringLocaleMap[entry];
    ...
    sLog.outString();
    if (min_value == MIN_BLIZZLIKE_STRING_ID)
        sLog.outString(">> Loaded %u blizzlike strings from table %s", count,table);
    else
...
const char *ObjectMgr::GetBlizzLikeString(int32 entry, int locale_idx) const
{
    ...
    if (entry > 0)
        sLog.outErrorDb("Entry %i not found in blizzlike_string table.",entry);
    else
        sLog.outErrorDb("blizzlike string entry %i not found in DB.",entry);
    return "<error>";
}
...
{
    LoadblizzlikeStrings(WorldDatabase,"db_script_string",MIN_DB_SCRIPT_STRING_ID,MAX_DB_SCRIPT_STRING_ID);
...
bool LoadblizzlikeStrings(DatabaseType& db, char const* table,int32 start_value, int32 end_value)
{
    ...
    return objmgr.LoadblizzlikeStrings(db,table,start_value,end_value);
}
