    ...
    if (entry->IsDungeon())
    {
        if (entry->IsRaid())
        {
            ...
            if (!player->isGameMaster() && !sWorld.getConfig(CONFIG_INSTANCE_IGNORE_RAID))
            {
                ...
                if (!group || !group->isRaidGroup())
                {
                    ...
                    player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetBlizzLikeString(810), mapName);
        ...
        if (!player->isAlive())
        {
            if (Corpse* corpse = player->GetCorpse())
            {
                ...
                if (!instance_map)
                {
                    player->GetSession()->SendAreaTriggerMessage(player->GetSession()->GetBlizzLikeString(811), mapName);
