...
namespace VMAP
{
    ...
    Vector3 VMapManager2::convertPositionToBlizzLikeRep(float x, float y, float z) const
    {
        Vector3 pos;
        const float mid = 0.5f * 64.0f * 533.33333333f;
        pos.x = mid - x;
    ...
    */
    bool VMapManager2::getObjectHitPos(unsigned int pMapId, float x1, float y1, float z1, float x2, float y2, float z2, float& rx, float &ry, float& rz, float pModifyDist)
    {
        ...
        if (isLineOfSightCalcEnabled())
        {
            ...
            if (instanceTree != iInstanceMapTrees.end())
            {
                ...
                resultPos = convertPositionToBlizzLikeRep(resultPos.x,resultPos.y,resultPos.z);
                rx = resultPos.x;
                ry = resultPos.y;
                rz = resultPos.z;
            }
    ...
    bool VMapManager2::getAreaInfo(unsigned int pMapId, float x, float y, float &z, uint32 &flags, int32 &adtId, int32 &rootId, int32 &groupId) const
    {
        ...
        if (instanceTree != iInstanceMapTrees.end())
        {
            ...
            // z is not touched by convertPositionToBlizzLikeRep(), so just copy
            z = pos.z;
        }
        return(result);
    }
